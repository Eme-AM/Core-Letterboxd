...
public class MailService {
    ...
    public void sendPasswordRecoveryEmail(String toEmail, String verificationCode) throws MessagingException {
        MimeMessage mimeMessage = javaMailSender.createMimeMessage();
        MimeMessageHelper helper = new MimeMessageHelper(mimeMessage, true);

        helper.setFrom("letterboxdcore@gmail.com");
        helper.setTo(toEmail);
        helper.setSubject("Recuperación de Contraseña - Letterboxd Core");
        
        String htmlBody = "<html>" +
                "<body style=\"font-family: Arial, sans-serif; line-height: 1.6; color: #333;\">" +
                "<div style=\"max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;\">" +
                "<h2 style=\"color: #ff6b35;\">Recuperación de Contraseña</h2>" +
                "<p>Hemos recibido una solicitud para restablecer tu contraseña en Letterboxd Core.</p>" +
                "<div style=\"background-color: #f9f9f9; padding: 15px; border-radius: 5px; margin: 20px 0;\">" +
                "<p><strong>Tu código de verificación es:</strong></p>" +
                "<h1 style=\"color: #ff6b35; font-size: 32px; text-align: center; margin: 10px 0;\">" + verificationCode + "</h1>" +
                "</div>" +
                "<p>Este código es válido por 15 minutos. Si no solicitaste este restablecimiento, puedes ignorar este correo.</p>" +
                "<p style=\"font-size: 14px; color: #666;\">¡Gracias por usar Letterboxd Core!</p>" +
                "</div>" +
                "</body>" +
                "</html>";
        ...
        helper.setText(htmlBody, true);
        javaMailSender.send(mimeMessage);
        System.out.println("Email de recuperación de contraseña enviado exitosamente.");
    }
